	...
	methods: {
		...
		addViews() {
			...
			if (!view.meta.hidden) {
				...
				if (view.meta.cache && view.meta.name) {
		...
		onHandleContextmenu(event, view) {
			...
			this.contextmenu = [
				{ type: 'refresh', name: '刷新', visible: this.isShowContextmenuItem(view, 'refresh') },
				{ type: 'close', name: '关闭', visible: this.isShowContextmenuItem(view, 'close') },
				{
					type: 'close-others',
					name: '关闭其他',
		...
		isShowContextmenuItem(view, type) {
			...
			switch (type) {
				...
				case 'close-all':
					return list.some((item) => !item.meta || !item.meta.pin);
				case 'close-left':
					return leftIndex > -1 && leftIndex < index;
				case 'close-right':
					return rightIndex > -1 && index < list.length - 1 - rightIndex;
				default:
					return false;
			}
		},
		...
		async onHandleContextmenuItem(view, type) {
			...
			switch (type) {
				case 'refresh':
					await this.$router.replace(`/refresh${view.fullPath}`);
					break;
				case 'close':
				...
				case 'close-others':
					...
					break;
				case 'close-all':
					if (view.path !== this.$route.path && view.meta && view.meta.pin) {
						await this.$router.push(view.path);
					} else {
