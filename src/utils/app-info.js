export default function getAppInfo(log) {
	...
	const time = env === 'development' ? devUpdateTime : prodUpdateTime;

	if (log) {
		const pd = 'padding:2px 4px';
		const br = (dir, size) => `border-top-${dir}-radius:${size}px;border-bottom-${dir}-radius:${size}px`;
		const nameStyle = `${pd};color:#fff;background:#555555;${br('left', 3)}`;
		const versionStyle = `${pd};color:#fff;background: #007ec6`;
		const timeStyle = `${pd};color:#fff;background:#dfb317`;
		const envStyle = `${pd};color:#fff;background:#33cc11;${br('right', 3)}`;
		console.log(`%c${name}%cv${version}%c${time}%c${env}`, nameStyle, versionStyle, timeStyle, envStyle);
	}
